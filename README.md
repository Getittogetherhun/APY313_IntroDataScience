...
## Purpose
  ...
  Through this information on Gender, Race, and Script Dialogue, my research question is *Does the combination of gender and race have a significant impact on the amount of words spoken by a character ?* As we've seen from test like DuVernay, speaking parts can potentially be indicative of representation. Since the amount of speaking parts could be central to representation in film, hypothetically, a characters race and gender could influence it. The null hypothesis here is that there is no significant difference in medians of the intersection of gender and race and speaking parts. The alternative hypothesis is that there is a significant difference (whether that be increase or decrease) in the medians between of the intersection of gender and race and speaking parts.To measure speaking part I utilized the polygraph words variable in the [**Harvard's Racial lines**](https://txtlab.org/wp-content/uploads/2019/01/Racial_Lines_2018.pdf)data set. Polygraph words are an automated process that extracts character dialogue from a screenplay [@DVN/KERZQY_2018].
...
## Collecting Data
  ...
  The data set is from [**Harvard's Racial lines**](https://txtlab.org/wp-content/uploads/2019/01/Racial_Lines_2018.pdf) data set. The [**Racial lines**](https://txtlab.org/wp-content/uploads/2019/01/Racial_Lines_2018.pdf) data set originates from a previous data set of screenplays with [**780 films**](https://pudding.cool/2017/03/film-dialogue/) on gender and dialogue conducted by Hannah Anderson and Matt Daniel at the Pudding [@Anderson2016-pd]. The [**Racial lines**](https://txtlab.org/wp-content/uploads/2019/01/Racial_Lines_2018.pdf) data set took the data set from The Pudding and incorporated the variable of Race [@DVN/KERZQY_2018]. During the process of data cleansing, I removed several variables that were not necessary for the data analysis being conducted.I then used the wilcox.test() function to generate test statistics to use in my analysis.
...
```
  ...
  With this table we can observe that we failed to reject the null hypothesis in all cases except for the comparison between Latinx Women and Latinx Men. In the case of Latinx characters, the p-value (0.002626) is less than the set alpha level (α = .05) so we reject the null hypothesis.
...
# Conclusion 
  ...
  With new concepts such as The DuVernay Test up and coming it will be interesting to see if speaking parts could possibly increase or decrease positive representation in film for minorities. For this study, the results supports that there is a relationship between Gender+Race and polygraph word count only for Latinx characters. These results also support that there is no relationship between the intersection of Gender+Race and polygraph word count for any other racial/ethnic group listed in the study. With the current data I found, so far we have information on the demographics of characters, age, and extracted words. In the future more data is needed on maybe the fiscal success of the films,films including Near Eastern Women, the ratings, and the amount of dialogue that was pertinent to the characters story only. Although probably rather very difficult and time consuming, I hope to see more intersectionality analysis when determining representation in film. 
